...
function callForgotPassAPI(event) {
  ...
  fetch('https://timsachnhabe-be.vercel.app/api/auth/forgot-password', {
    ...
    .then(async (res) => {
      // Use async to use await res.json() or res.text()
      ...
      if (res.ok) {
        ...
        window.location.href =
          '../pages/quenMK2Ad.html?email=' + encodeURIComponent(email);
      } else {
        // Status not 200 (e.g., 400, 500)
    ...
    .catch((error) => {
      ...
      alert(
        'Có lỗi xảy ra khi gửi yêu cầu. Vui lòng kiểm tra kết nối và thử lại.'
      );
    });
