...
if (!resetEmail) {
  const errorMessage =
    'Không tìm thấy thông tin email. Vui lòng quay lại trang trước.';
...
function callResetPassAPI(event) {
  ...
  if (!passwordRegex.test(newPassword)) {
    resetErrorDiv.textContent =
      'Mật khẩu mới không hợp lệ! Phải có ít nhất 8 ký tự, bao gồm chữ hoa, chữ thường và số.';
    resetErrorDiv.classList.remove('hidden');
    return false;
  }
  ...
  fetch('https://timsachnhabe-be.vercel.app/api/auth/reset-password', {
    ...
    .then(async (res) => {
      ...
      if (res.ok) {
        // Status 200 OK - Đặt lại mật khẩu thành công
        ...
        window.location.href =
          '../pages/quenMK3Ad.html?email=' + encodeURIComponent(resetEmail);
      } else {
        // Status not 200 (e.g., 400, 404, 500) - Đặt lại mật khẩu thất bại
    ...
    .catch((error) => {
      ...
      alert(
        'Có lỗi xảy ra khi gửi yêu cầu đặt lại mật khẩu. Vui lòng kiểm tra kết nối và thử lại.'
      );
    });
